...
function PT_GetCategoryByID($video_id = '', $site, $add_views = 0, $likes_dislikes = 0, $run_query = 1, $short_id = 0) {
    global $pt, $db, $categories;
...
function PT_SiteImage($data = ''){
    global $pt, $db;
    $data= PT_Secure($data);
    $allowed           = 'png';
    $new_string        = pathinfo($data, PATHINFO_FILENAME) . '.' . strtolower(pathinfo($data, PATHINFO_EXTENSION));
    $extension_allowed = explode(',', $allowed);
    $file_extension    = pathinfo($new_string, PATHINFO_EXTENSION);
    $dir      = "themes/" . $pt->config->theme . "/img/";
    $filename = $dir . "$new_string";
