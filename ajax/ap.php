...
if($first == 'create-site'){
    $user = $pt->user->id;
   $base_link =  PT_Link('');
    $site_link = $base_link.$_POST['site_name'];
    $mysite = $_POST['site_name'];
    $chk_site = $db->where('site_name',$mysite)->getOne(T_SITE);
   if(!empty($chk_site)){
    return $data['error'] = 'Site Already Exist';
    return response()->json($data);
   }
    if (!empty($_POST['site'])) { 
        $name = $_FILES['file']['name'];
        $target_dir =  "themes/" . $pt->config->theme . "/img/";
        ...
        $insert_array = array(
            'user_id' => $user,
            'site_name' => PT_Secure($_POST['site_name']),
            'description' => PT_Secure($_POST['description']),  
            'logo' => $image,
            'link' => $site_link
        );
        // TO define the table name go to the asset/table.php & script/asset/table.php
        $insert = $db->insert(T_SITE, $insert_array);
        // $data = array('status' => 200);
