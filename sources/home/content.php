...
foreach ($trending_data as $key => $video) {
    $video = $pt->video =  PT_GetVideoByIDs($video, $split[2], 0, 0, 0);
    ...
    if (!empty($posts)) {
        foreach ($posts as $key => $post) {
            ...
            $html_posts .= PT_LoadPage('home/article_list', array(
                ...
                'CAT' => ($post->category),
                'URL' => PT_Link($split[2].'/articles/read/' . PT_URLSlug($post->title,$post->id)),
...
if ($pt->config->shorts_system == 'on') {
    ...
    foreach ($videos as $key => $video) {
        $video = PT_GetVideoByIDs($video,$split[2], 0, 0, 0);
        $video->url                = PT_Link($split[2].'/shorts/' . $video->video_id);
        $video->load_ajax = $video->video_id;
        if ($pt->config->seo_link == 'on') {
            $video->url                = PT_Link($split[2].'/shorts/' . PT_Slug($video->title, $video->video_id));
...
function PT_GetVideoByIDs($video_id = '', $site, $add_views = 0, $likes_dislikes = 0, $run_query = 1, $short_id = 0) {
    ...
    if (!empty($get_video)) {
        ...
        $get_video->url                = PT_Link($site.'/watch/' . $get_video->video_id);
        $get_video->ajax_url                = '?link1=watch&id='.$get_video->video_id;
        if ($pt->config->seo_link == 'on') {
            $get_video->url                = PT_Link($site.'/watch/' . PT_Slug($get_video->title, $get_video->video_id));
        }
        if ($get_video->is_short == 1) {
            $get_video->url                = PT_Link($site.'/shorts/' . $get_video->video_id);
            $get_video->ajax_url                = '?link1=shorts&id='.$get_video->video_id;
            if ($pt->config->seo_link == 'on') {
                $get_video->url                = PT_Link($site.'/shorts/' . PT_Slug($get_video->title, $get_video->video_id));
            }
        }
