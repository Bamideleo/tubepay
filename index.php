...
$tmpURL = $_SERVER['SERVER_NAME'];
if($actual_link =='https://localhost/tubepay/'){
    require_once('./assets/init.php');
    // $page = 'vendor';
    include("./themes/youplay/layout/vendor/content.html");
  
}

elseif($_GET['link1'] == "login"){  
    require_once('./assets/init.php');
    $page=$_GET['link1'];
...
$page = 'home';
    ...
    if (!empty($url_key)) {
        $page = $url_key;
        $split_search = explode("?",$page);
        $my_search = $split_search[1];
        if($my_search){
         $page = $split_search[0];  
        }
    ...
    if ($split[3] == "v") {
        $video_short_id = PT_Secure($split[4]);
        $get_video = PT_GetCategoryByID($video_short_id, $split[2], 0, 0, 1, 1);
        if (!empty($get_video)) {
